...
def get_dbp_ents(endpoint, query_template, query_params, lang):
    ...
    :param query_params: the parameters of the queries such as the entity type and the music genre keyword
    ...
    while (True):
        print('Offset :', offset)
        query = query_template.substitute({'entity': query_params['entity_type'], 'offset': offset, 'genre_keyword': query_params['genre_keyword'], 'other_lang_cond': query_params['other_lang_cond']})
        #print(query)
...
qt_alias = Template("""SELECT ?entity, ?alias{{
        SELECT ?entity, ?alias
            WHERE {
                ...
                FILTER EXISTS {
                    ?entity <http://dbpedia.org/ontology/$genre_keyword> ?genre
                }.
