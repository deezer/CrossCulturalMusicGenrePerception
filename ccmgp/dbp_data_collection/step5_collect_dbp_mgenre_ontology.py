...
def process_query(query, sparql_dbp, relations, seeds, genres, inv=False):
    ...
    :param relations: dictionary with genre relations that will be updated
    ...
    :param inv: a seed genre could be the source of an edge in the DBpedia ontology or the target; depending on the role it plays the relations dictionary is updated differently
    """
...
def create_dbp_multigraph(genres, relations, genre_aliases):
    """Create the DBpedia music genre ontology
    :param genres: the list of discovered genres that will be ontology concepts
    :param relations: the list of music genre relations
...
df = pd.read_csv(opj(utils.CORPUS_FILE_PATH), index_col='id')
...
# either because they were marked as MusicGenre in DBpedia or they are aliases
# of MusicGenre entities or they were associated as genres to corpus items
...
# Create and save the ontology as a networkx MultiGraph
